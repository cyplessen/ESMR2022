...
subtitle: "2. Descriptive Specification Plot"
...
library(tidyverse) # Probably something, at this point I am not sure anymore :)  
...
```{r}
specifications_full <- read_csv2("data/specifications_full.csv")
data_sim <- read_csv2("data/data_sim.csv")
```
...
##### Create specification tile plot 
...
p1_rel <- ggplot(data = plotdata_rel, 
       aes(x = xvar, 
           ...
           fill = fill_manual)) +
  geom_raster() + 
  geom_hline(yintercept = c(6, 8, 12, 15, 20, 23, 27, 32) + 0.5) +  # Change here
...
ma_for_forest_3  <- specifications_full_rel %>% 
  arrange(mean) %>% 
  slice(44) %>%  mutate(label = "Meta-Analysis B")

ma_for_forest_4  <- specifications_full_rel %>% 
  arrange(mean) %>% 
  slice(150) %>%  mutate(label = "Meta-Analysis C")
...
```{r}
specifications_full <- read_csv2("data/specifications_full_null.csv")
data_sim <- read_csv2("data/data_sim_null.csv")
```
...
```{r}
write.csv(plotdata, "data/plotdata_null.csv")
```
...
p1_rel <- ggplot(data = plotdata_rel, 
       aes(x = xvar, 
           ...
           fill = fill_manual)) +
  geom_raster() + 
  geom_hline(yintercept = c(6, 10, 13, 16, 20, 23, 27, 32) + 0.5) +  # Change here
  scale_x_continuous(position = "bottom") +
  scale_y_discrete(labels = ylabels) +
  scale_fill_manual(
    values = c("white", cols_rev)) +
...
ylimit <- c(-.5, 2)
#y_breaks_forest <- round(seq(from = round(ylimit[1], 1), to = round(ylimit[2], 1), by = 0.5), 2)
y_breaks_forest <- seq(-.25, 1.75, 0.25)
...
## Save Plot
```{r }
ggsave("figures/descriptive_specification_null.png", 
       p_rel, 
       width = 10, 
       height = 7.5,
       dpi = "retina"
