...
```{r}
for(j in 1:iter) {
  # Draw randomly new effect sizes (Cohens d) from a normal distribution with SE = 1/sqrt(N - 3) 
...
boot_plot <- 
  ...
  geom_ribbon(aes(x =  xvar, ymin = boot_lb*10, ymax = boot_ub*10),  # multiplication just to visualize this simulated example!!!
...
## Load data
```{r}
dat_boot <- read_csv2("data/data_sim_null.csv")
head(dat_boot)
```
...
```{r}
# Function that takes the ids of studies for each unique specification regarding the included studies (Which factors), 
# and computes the resulting summary effect for each of the 8 "How factors".
spec_list  <- function(ids, data) {
  temp <- data[ids, ]
  spec <- 
    c(rma(yi = yi,  # FE
...
dat_boot_null <- data.frame(xvar = 1:no_spec, obs = sort(specifications_inferential_spec_curve$mean), boot_lb, boot_ub)
write.csv2(file = "data/inferential_data_null.csv", dat_boot)
```
...
boot_plot_null <- 
  ...
  geom_ribbon(aes(x =  xvar, ymin = boot_lb *10, ymax = boot_ub*10), # multiplication x10 for visualization purposes only!!!
              fill = "gray", color = "black", lty = "dotted", alpha = 0.7, size = 0.25) +
  geom_line(col = "firebrick", size = 0.5) +
  geom_hline(yintercept = 0, linetype = 2, size = 0.25) +
  scale_x_continuous(name = "Specification number") +
